...
const orders = computed(()=> store.getters.getOrderServe)
...
<template>
    <section class="md:my-16 md:mx-16 my-8 mx-8">
        ...
        <div class="flex flex-row gap-3 flex-wrap place-content-center max-w-[100%]" v-if="orderFoods.length > 0">
            <OrderComponent v-for="(order, index) in orderFoods" :key="index" :tableID="order.tableID" :orderAt="order.orderAt" role="Waiter" :menus="orders" @submit="updateOrder" />
        </div>
