...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    if (isValidTokenAndRole($conn, $obj->token, 'MANAGER')) {
        if ($conn->query("INSERT INTO gacha_banner (name, description, cost, expire) VALUES ('{$obj->gacha->name}', '{$obj->gacha->description}', '{$obj->gacha->cost}', '{$obj->gacha->expire}')")) {

            $payload = [
                'result' => '',
                'message' => 'Add gacha successful',
                'status' => 0,
            ];
        } else {
            $payload = [
                'result' => '',
                'message' => 'Add gacha failed',
                'status' => 1,
            ];
        }
    } else {
