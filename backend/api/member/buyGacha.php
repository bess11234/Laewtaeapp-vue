...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    if (isValidTokenAndRole($conn, $obj->token, 'MEMBER')) {        
        ...
        if ($conn->query("INSERT INTO user_discount (userID, menuID, discount, code, expire) VALUES ($userID, {$obj->gacha->menuID}, {$obj->gacha->discount}, '$code', ADDDATE(CURDATE(), INTERVAL 1 YEAR) )")) {
            $conn->query("UPDATE users SET points=points-{$obj->gacha->cost} WHERE userID=$userID ");

            $payload = [
                'result' => '',
                'message' => "Gacha {$obj->gacha->menu} ({$obj->gacha->rarity})",
                'status' => 0,
            ];
        } else {
            $payload = [
